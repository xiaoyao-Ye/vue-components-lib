...
const mainFilePath: string = path.join('packages/components', compName, `${compName}.vue`);
...
const detectPublic = async (): Promise<void> => {
  ...
  logInfo(`${config.prefixU}${displayName} 组件创建完成 🎉🎉🎉` + '\n');
};
...
async function generate(): Promise<[void, void, void, void, void]> {
  updatedFiles.push(
    ...
    `packages/components/${compName}/__test__/${compName}.spec.ts`,
  );
...
async function updateComponentEntry(): Promise<void> {
  const entryFilePath: string = path.resolve(__dirname, '../components/components.ts');
  ...
  content =
    ...
    '\n' +
    '\n' +
    `export { ${config.prefixU}${displayName} } from './${compName}'` +
    '\n' +
...
async function updateStyleEntry(): Promise<void> {
  const entryFilePath: string = path.resolve(__dirname, '../theme/index.scss');
  let content: string = (await fsExtra.readFile(entryFilePath)).toString();
...
async function incrementTest(): Promise<void> {
  const outputDir: string = path.resolve(__dirname, `../components/${compName}/__test__`);
...
async function superEjsGerenateDir(outputDir: string, tplDir: string): Promise<void> {
  return await superEjs.gerenateDir(
    ...
    tplDir,
    {
      prefixU: config.prefixU,
      prefixL: config.prefixL,
      name: compName,
      displayName,
      // changeCase
    },
